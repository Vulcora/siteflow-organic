...
const CaseStudiesPage: React.FC<CaseStudiesPageProps> = ({ onNavigate, onSelectCase }) => {
  ...
  // Filter case studies based on search
  const filteredCases = useMemo(() => {
    if (!searchQuery.trim()) return allCaseStudies;

    const query = searchQuery.toLowerCase();
    return allCaseStudies.filter(cs =>
      cs.title[lang].toLowerCase().includes(query) ||
      cs.excerpt[lang].toLowerCase().includes(query) ||
      cs.client.industry[lang].toLowerCase().includes(query) ||
      cs.tags.some(tag => tag.toLowerCase().includes(query))
    );
  }, [allCaseStudies, searchQuery, lang]);
        ...
        <div className="container mx-auto px-6">
          ...
          <div className="max-w-xl mx-auto mb-12">
            ...
            {searchQuery && (
              <p className="text-sm text-slate-500 mt-2 text-center">
                {filteredCases.length} {t('caseStudies.resultsFound')}
              </p>
            )}
          </div>
          ...
          {paginatedCases.length > 0 ? (
            <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
              {paginatedCases.map((caseStudy, index) => (
                <CaseStudyCard
                  key={caseStudy.slug}
                  caseStudy={caseStudy}
                  onClick={() => onSelectCase(caseStudy.slug)}
                  index={index}
                />
              ))}
            </div>
            ...
            <div className="text-center py-12">
              <p className="text-slate-500 text-lg">{t('caseStudies.noResults')}</p>
            </div>
          )}
